...
#' @param pdfnames Optional. Names and paths of the 4 PDF generated by the function. Applies only if save_plot is TRUE. Default names are generated if not specified.
...
dewlap_neural <- function(specdata, vars, nRepet = 1000, plot_success = T, plot_importance = T, save_plot = T, pdfnames, seed, font) {
  ...
  if(plot_success) {
    ...
    if(save_plot) ggsave(pdfnames[1], p1, device = "pdf", width = 4, height = 2.5, family = font) else print(p1)

    if(save_plot) ggsave(pdfnames[2], p2, device = "pdf", width = 4, height = 2.5, family = font) else print(p2)
  ...
  if(plot_importance) {
    ...
    if(length(grep("wl", names(importanceTable))) != 0) {
      ...
      if(save_plot) pdf(pdfnames[3], width = 5, height = 4, family = font)
    ...
    if(save_plot) pdf(pdfnames[4], width = 3, height = 4, family = font)
